...
function Settings() {
  ...
  const getInfo= async ()=>{
    try{
        ...
        setImgUrl(require(`C:/Users/apple pro/OneDrive/Desktop/e-commerce/ms-auth/images/${res.data.image}`))
        setAdr({
...
const updateInfo= async ()=>{
  setLoading(true)
  try{
    let res = await axios.patch(urlUpdate,profilInfo,{withCredentials:true})
  }
  catch(e){
    console.log(e)
    setLoading(false)
    ...
    <div className=" w-full h-[80%]">
      ...
      <div className="flex ss:mt-5 mt-0 flex-col gap-2 justify-center items-center py-10 ">
            ...
            <div className=" ss:w-[70%] w-full mt-3 flex flex-col gap-1">
              ...
              <div className=" rounded-[4px] border-[1px] border-[#A08C89] flex flex-row w-full">
                <input
                  ...
                  onChange={(e) => {
                    setProfileInfo({ ...profilInfo, numberPhone: e.target.value });
                  }}
                />
              </div>
            </div>
        ...
        <div className=" flex ss:justify-start justify-center  items-center mt-10 w-[70%]">
          <button
            onClick={()=>{updateInfo();updateImage()}}
      ...
      <div className=" w-full px-8 h-[30%] flex ss:flex-row flex-row-reverse justify-between items-center">
        ...
        <p onClick={()=>{logoutUser()}} className="text-black hover:text-green-500 text-[30px] gap-2 flex justify-center items-center  hover:cursor-pointer">
        <BiLogOutCircle /> 
        ...
        <div className=" flex flex-col gap-1">
          <p onClick={()=>navigate("/terms&condition")}  className=" hover:text-green-500  text-black hover:cursor-pointer hover:underline  hover:underline-offset-1">
            Terms and Conditions
          </p>
          <p onClick={()=>navigate("/privacy policy")} className=" hover:text-green-500 text-black hover:cursor-pointer hover:underline hover:underline-offset-1">
            Privacy Policy
          </p>
          <p onClick={()=>navigate("/shipping policy")} className=" hover:text-green-500 text-black hover:cursor-pointer hover:underline hover:underline-offset-1">
            Shipping Policy
          </p>
          <p onClick={()=>navigate("/refunds")} className= " hover:text-green-500 text-black hover:cursor-pointer hover:underline hover:underline-offset-1">
