...
export const Prod = ({
  ...
  const deleteProduct = async () => {
    try {
      ...
      console.log(res);
      let newArr = products.filter((it) => item !== it);
      setProducts(newArr);
    } catch (e) {
      console.log(e);
    }
...
function ProductList() {
  ...
  const getSearched = async () => {
    try {
      ...
      let res = await axios.get(`${urlSearch}?productNameLike=${search}`, {
    ...
    <>
      <div class="h-[10%] flex flex-row w-full mx-auto p-4">
        <div class=" flex-1 flex items-center space-x-4">
          <div class="flex items-center space-x-2">
            <label for="lowerPrice" class="text-gray-600">
              Price:
            </label>
            <input
              onChange={(e) =>
                e.target.value === ""
                  ? setLessThan(0)
                  : setLessThan(parseFloat(e.target.value))
              }
              type="number"
              id="lowerPrice"
          ...
          <div class="flex items-center space-x-2">
            <input
              onChange={(e) =>
                e.target.value === ""
                  ? setGreaterThan(0)
                  : setGreaterThan(parseFloat(e.target.value))
              }
              type="number"
              id="upperPrice"
              placeholder="Upper than"
              className=" outline-none border border-gray-300 px-2 py-1 rounded"
            />
          </div>
        ...
        <div className=" flex-2 ">
          <div className="px-4 flex w-[100%]  justify-start items-center relative">
            <input
              ...
              className=" w-[100%] px-10 drop-shadow-md py-2 rounded-2xl border-color border-[0.25px] outline-none"
              type="search"
              placeholder="Search..."
            />
            <img
              src={searchsvg}
              className="absolute ml-2 w-6 h-6"
              alt="search"
            />
          </div>
        </div>
      ...
      <div className=" overflow-scroll h-[90%] flex justify-start items-start flex-row flex-wrap gap-10 px-10 py-5 ">
        {toggle && (
          <div className=" fixed left-0 top-0 w-[100%] h-screen bg-black bg-opacity-60 backdrop-blur-sm z-[9999] flex justify-center items-center  ">
            <UpdateProductPopup
              ...
              setBool={setBool}
              categoryName={infoPopup.categoryName}
        ...
        {(lessThan !== 0 ||
          greaterThan !== 0 ||
          selectedOption !== null) &&
            filtredData.filter((item) => item.productName.includes(search)).map((item, index) => (
              <Prod
                key={item.productId}
                products={products}
                setProducts={setProducts}
